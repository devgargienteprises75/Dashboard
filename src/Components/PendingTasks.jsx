...
const PendingTasks = ({ data, className }) => {
  ...
  const pending_details = data.pending_details
      ...
      {allPendingItems.length === 0 ? (
        ...
        <div className='space-y-3 max-h-96 overflow-y-auto'>
          {allPendingItems.map((item) => (
            <div key={item.id} className='flex items-start gap-4 p-3 bg-yellow-50 rounded-lg border border-yellow-200'>
