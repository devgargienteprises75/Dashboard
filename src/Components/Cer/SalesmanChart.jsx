...
const SalesmanChart = ({ cerData = [] }) => {
  ...
  const key = findCandidateKey(dataObj, ['salesman', 'sales'])
  ...
  if (!sorted.length) return (
    <div className='bg-white rounded-lg p-6 shadow-sm border border-gray-100'>
      <h2 className='text-lg font-bold text-gray-900 mb-4'>Exits by Salesman</h2>
    ...
    <div className='bg-white rounded-lg p-6 shadow-sm border border-gray-100'>
      ...
      <ResponsiveContainer width="100%" height={260}>
        <BarChart data={sorted} layout="vertical" margin={{ left: 40 }}>
          ...
          <Tooltip />
          <Bar dataKey="value" fill="#4366FF" radius={[8, 8, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
